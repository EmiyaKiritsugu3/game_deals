...
const SNAPSHOTS_DIR = path.join(__dirname, 'visual.spec.ts-snapshots');
...
test.describe('Visual regression', () => {
  ...
  test('2. Search results page renders correctly', async ({ page }) => {
    ...
    const fp = path.join(
      SNAPSHOTS_DIR,
      '2-Search-results-page-renders-correctly-chromium-linux.png'
    );
    fs.mkdirSync(SNAPSHOTS_DIR, { recursive: true });
    fs.writeFileSync(fp, buf);
    expect(fs.statSync(fp).size).toBeGreaterThan(1000);
  ...
  test('3. Shared wishlist page renders correctly', async ({ page }) => {
    ...
    const fp = path.join(
      SNAPSHOTS_DIR,
      '3-Shared-wishlist-page-renders-correctly-chromium-linux.png'
    );
    fs.mkdirSync(SNAPSHOTS_DIR, { recursive: true });
    fs.writeFileSync(fp, buf);
    expect(fs.statSync(fp).size).toBeGreaterThan(1000);
