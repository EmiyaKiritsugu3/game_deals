...
if (!feature) {
  const git = await agent(`Read current git: branch, recent 5 commits, diff --stat main, TODOs in changed files. Infer feature. 1 sentence.`, { label: 'discover' });
  feature = git || 'infer context';
  console.log(`Inferred: ${feature}`);
}

phase('Spec+Plan');
const plan = await agent(`Read project (package.json, CLAUDE.md, src/ structure, existing tests). Write spec + task list for: ${feature}\n\nOutput spec + tasks file + deps.`, {
  label: 'plan',
  schema: {
    type: 'object',
    properties: {
      tasks: { type: 'array', items: { type: 'object', properties: { id: { type: 'string' }, desc: { type: 'string' }, file: { type: 'string' }, deps: { type: 'array', items: { type: 'string' } } }, required: ['id', 'desc'] } },
...
phase('Smoke');
const smoke = await agent(`Run full test suite. Any fails, fix them. Repeat until entire suite passes.`, {
  label: 'smoke',
  schema: { type: 'object', properties: { pass: { type: 'boolean' } }, required: ['pass'] },
});
if (!smoke?.pass) { console.log('❌ Smoke failed'); return; }

phase('SlopGate');
const slop = await agent(`Review all changed files for common AI slop patterns:
...
For each finding, output path:line: <pattern> — <fix>. If no slop, output CLEAN.`, { label: 'slop-check' });
if (slop && slop !== 'CLEAN') {
  console.log(`Slop found:\n${slop}`);
  const fixed = await agent(`Fix issues:\n\n${slop}\n\nApply fixes following project conventions.`, { label: 'slop-fix' });
  if (fixed) console.log('Slop auto-fixed');
  const reSmoke = await agent(`Run full test suite. Any fails, fix them. Pass/fail only.`, {
    label: 're-smoke',
    schema: { type: 'object', properties: { pass: { type: 'boolean' } }, required: ['pass'] },
  });
  if (!reSmoke?.pass) { console.log('❌ Post-slop smoke failed'); return; }
...
phase('Deliver');
const branch = await agent(`What git branch are we on? Output just branch name.`, { label: 'branch-name' });
const isMain = branch === 'main' || branch === 'master';

if (isMain || args?.asPr) {
  const prBody = await agent(`Generate PR description for changes on ${branch} targeting main.`, { label: 'pr-body' });
  await agent(`Create PR:\nTitle: feature/${feature}\nBody: ${prBody}\n\nUse gh CLI.`, { label: 'create-pr' });
  console.log(`✅ PR created from ${branch}`);
} else {
  await agent(`Commit all changes with conventional commit describing: ${feature}\n\nPush to origin/${branch}.`, { label: 'deliver-commit' });
