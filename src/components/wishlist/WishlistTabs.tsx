...
export default function WishlistTabs({
  ...
  const baseTab =
    'flex items-center gap-2.5 px-6 py-3 border-none bg-none text-muted-foreground font-bold text-[0.95rem] cursor-pointer relative transition-colors duration-200 hover:text-foreground focus-visible:outline-2 focus-visible:outline-primary focus-visible:outline-offset-2 focus-visible:rounded';
  ...
  return (
    <div className="flex gap-4 mb-8 border-b border-border/50 pb-2">
      <button
        ...
        aria-current={activeTab === 'wishlist' ? 'true' : undefined}
      >
        <List size={20} />
      ...
      <button
        ...
        aria-current={activeTab === 'alerts' ? 'true' : undefined}
      >
        <Bell size={20} />
