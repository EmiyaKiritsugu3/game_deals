...
export function AuthBrandPanel() {
  return (
    <div className="relative hidden overflow-hidden bg-gradient-to-br from-card via-card/80 to-background lg:flex lg:flex-col lg:justify-between lg:p-8">
      ...
      <div
        className="pointer-events-none absolute -right-20 -top-20 size-72 rounded-full opacity-40 blur-3xl animate-float-slow"
        style={{
          background: 'radial-gradient(circle, oklch(0.78 0.2 145 / 0.6), transparent 60%)',
        }}
      />
      <div
        className="pointer-events-none absolute -bottom-24 -left-24 size-80 rounded-full opacity-30 blur-3xl animate-float-slower"
        style={{
          background: 'radial-gradient(circle, oklch(0.78 0.16 70 / 0.5), transparent 60%)',
        }}
      />
      {/* Slow shimmer sweep across the panel */}
      <div
        className="animate-auth-shimmer pointer-events-none absolute inset-y-0 -left-1/3 w-1/2 opacity-40"
        style={{
          background:
            'linear-gradient(100deg, transparent, oklch(0.78 0.2 145 / 0.25), transparent)',
        }}
      />
      {/* Subtle grid texture */}
