...
/**
 * Brand SVG icons for social login buttons. We inline these rather than
 * adding an icon library, per the "no new dependencies" directive.
 */
...
interface SocialProviderConfig {
  ...
  /** Tailwind classes for the icon color on hover. */
  hoverCls: string;
}
...
export function SocialLoginButtons({ onSignIn, loading, disabled }: SocialLoginButtonsProps) {
  return (
    <div className="grid grid-cols-3 gap-2.5">
      {PROVIDERS.map(({ provider, label, icon: Icon, hoverCls }) => {
        ...
        return (
          <button
            ...
            className={cn(
              'group relative flex h-11 flex-col items-center justify-center gap-1 rounded-xl border border-border/50 bg-card/40 backdrop-blur-md transition-all duration-300',
