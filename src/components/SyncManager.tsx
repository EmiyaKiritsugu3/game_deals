  ...
  useEffect(() => {
    ...
    const syncToCloud = async () => {
      ...
      if (wishlist.length > 0) {
        ...
        await supabase?.from('wishlists').upsert(wishlistData, { onConflict: 'user_id,game_id' });
      }
      ...
      if (alerts.length > 0) {
        ...
        await supabase?.from('price_alerts').upsert(alertsData, { onConflict: 'user_id,game_id' });
      }
    };
