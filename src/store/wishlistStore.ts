...
interface WishlistState {
  ...
  // Aliases for new-ui compatibility
  toggle: (gameID: string) => void;
  has: (gameID: string) => boolean;
  addRecentlyViewed: (_dealID: string) => void;
}
  ...
  persist(
    (set, get) => ({
      ...
      addToWishlist: (gameID: string) =>
        set((state) => ({
          wishlist: state.wishlist.includes(gameID) ? state.wishlist : [...state.wishlist, gameID],
        })),
      ...
      toggle: (gameID: string) => get().toggleWishlist(gameID),
      has: (gameID: string) => get().isInWishlist(gameID),
      addRecentlyViewed: () => {},
    }),
    {
