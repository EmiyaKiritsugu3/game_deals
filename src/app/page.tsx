...
export default function Home() {
  ...
  const stats = React.useMemo(() => {
    ...
    // ⚡ Bolt Optimization: Use single-pass loop instead of list.reduce() and Math.max(...list.map())
    // to avoid O(N) memory allocation and O(2N) iteration overhead on potentially large deal lists.
    let sumSavings = 0;
    let topSavings = 0;

    for (const d of list) {
      sumSavings += d.savingsNum;
      if (d.savingsNum > topSavings) {
        topSavings = d.savingsNum;
      }
    }

    const avgSavings = list.length ? Math.round(sumSavings / list.length) : 0;
    const topDiscount = list.length ? Math.round(topSavings) : 0;
