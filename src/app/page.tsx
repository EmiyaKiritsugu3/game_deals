...
export default function Home() {
  ...
  const stats = React.useMemo(() => {
    ...
    // ⚡ Bolt Performance: Single-pass iteration to calculate sum and max.
    // Avoids sequential reduce/map and Math.max spread which can cause
    // O(N) memory allocation and exceed the JS engine call stack size limit.
    for (const d of list) {
      sumSavings += d.savingsNum;
      if (d.savingsNum > maxDiscount) maxDiscount = d.savingsNum;
    }
    const avgSavings = list.length ? Math.round(sumSavings / list.length) : 0;
    const topDiscount = list.length ? Math.round(maxDiscount) : 0;
