...
export default function Home() {
  ...
  const stats = React.useMemo(() => {
    ...
    // ⚡ Bolt Performance: Calculate total savings and top discount in a single O(N) pass
    // instead of chaining .reduce and Math.max(...list.map()) to prevent O(N) allocations and call stack overflow.
    let totalSavings = 0;
    let topDiscount = 0;
    for (const d of list) {
      totalSavings += d.savingsNum;
      if (d.savingsNum > topDiscount) topDiscount = d.savingsNum;
    }
    const avgSavings = list.length ? Math.round(totalSavings / list.length) : 0;
    topDiscount = list.length ? Math.round(topDiscount) : 0;
