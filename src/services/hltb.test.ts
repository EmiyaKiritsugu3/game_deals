...
titles.forEach((t) => {
  ...
  assert.ok(
    res.mainStory >= 6 && res.mainStory <= 80,
    `Range failed for "${t}": mainStory=${res.mainStory}`
  );
});
...
titles.forEach((t) => {
  const res = estimatePlaytime(t);
  assert.ok(
    res.mainExtra >= res.mainStory,
    `Consistency failed: mainExtra(${res.mainExtra}) < mainStory(${res.mainStory}) for "${t}"`
  );
  assert.ok(
    res.completionist >= res.mainExtra,
    `Consistency failed: completionist(${res.completionist}) < mainExtra(${res.mainExtra}) for "${t}"`
  );
});
