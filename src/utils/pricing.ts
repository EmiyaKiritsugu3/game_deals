import { GameDeal, PriceHistoryPoint } from "@/types/game";
...
export function generateGreyMarketDeals(officialDeals: GameDeal[], dealIDRef: string): GameDeal[] {
    ...
    const shuffledShops = [...GREY_MARKET_SHOPS].sort((a, b) => (hash % parseInt(a.id)) - (hash % parseInt(b.id)));
...
export function generatePriceHistory(retailPrice: number, currentPrice: number, lowestPrice: number, seed: string): PriceHistoryPoint[] {
