...
## 2026-08-03 - Optimizing Array Processing\n**Learning:** Iterating over large arrays and performing `[...arr].sort(...)[0]` introduces unnecessary memory allocation due to spread syntax and (N \log N)$ complexity due to sorting. The `src/app/collections/[slug]/page.tsx` file had this anti-pattern in the `gameDataToDeal` loop causing unnecessary overhead during collection rendering.\n**Action:** Use single-pass (N)$ linear searches via the helper `getCheapestDeal` for finding the minimum/maximum in a data set to avoid O(N) heap allocations and sorting overhead, especially in iterative collection processing functions.
