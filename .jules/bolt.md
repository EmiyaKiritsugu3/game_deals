...
## 2024-10-18 - [Optimizing array mapping and batch processing in React Components]
**Learning:** Found that operations like `[...deals].sort(...)[0]` introduce O(N log N) sorting latency plus O(N) memory allocation overhead via the array spread. In large loops or server-side batches (like generating pages for game collections), this causes unnecessary CPU overhead. The proper solution is to use linear scans, e.g., using `getCheapestDeal` directly on the initial array, to reduce the time complexity to O(N) and space complexity to O(1).
**Action:** When finding extremes (min/max), strictly avoid cloning (`[...]`) followed by sorting. Opt instead for iterative `for` loops or linear helper functions like `getCheapestDeal()`.
