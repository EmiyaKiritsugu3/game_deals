...
## 2024-09-04 - Optimize array processing in Server Components
**Learning:** Sequential `.reduce()` and `Math.max(...list.map(...))` calls over potentially large datasets create unnecessary multiple O(N) iterations, allocate extra memory with `.map()`, and risk exceeding the JavaScript call stack size limit when spreading massive arrays.
**Action:** Replace multiple sequential array methods and spreads with a single-pass `for...of` loop to calculate aggregate stats simultaneously. This reduces memory allocation and ensures robust execution for large lists.
