...
## 2026-09-09 - Math.max optimization
**Learning:** Found an O(N) allocation + spreading bottleneck from using `Math.max(...arr.map(...))` sequentially after a `reduce()` across lists of arrays. Using single-pass loops calculates both totals and maximums directly within one O(N) step, bypassing array allocation.
**Action:** Optimize calculations with single-pass loops to prevent memory allocation and duplicate iterations.
