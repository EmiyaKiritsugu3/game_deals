...
## 2024-05-18 - Single-Pass Reductions
**Learning:** Sequential uses of `.map()`, `.reduce()`, or spread operators like `Math.max(...array.map(...))` create unnecessary O(N) memory allocations and increase time complexity by performing multiple iterations. Using `.map()` and spreading large arrays inside `Math.max()` can exceed the call stack.
**Action:** Replace multiple iterations with a single `for...of` loop to calculate aggregate stats (like average and maximum) simultaneously to ensure O(1) memory complexity and robust execution.
