...
## 2024-11-20 - [Avoid Server-Side Promise.all Waterfall]
**Learning:** Found a sequence of independent `await Promise.all(...)` calls for different data groups in a Server Component (`DealsHubPage`). Awaiting these sequentially creates a waterfall effect that unnecessarily increases Time To First Byte (TTFB).
**Action:** Always wrap independent concurrent data fetches into a single, top-level `Promise.all([Promise.all(...), Promise.all(...)])` to ensure maximum network parallelization and minimize server-side blocking time.
