...
## 2024-11-25 - Avoid O(N log N) sorting and array allocation in gameDataToDeal
**Learning:** `[...arr].sort(...)[0]` pattern in `src/app/collections/[slug]/page.tsx` was causing unnecessary memory allocations and an O(N log N) operation for finding minimum price.
**Action:** Replaced `[...arr].sort(...)[0]` with a call to the `getCheapestDeal` utility which finds the minimum value in O(N) without array allocation.
