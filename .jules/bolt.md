...
## 2024-05-24 - Avoid Math.max with array spread on large arrays
**Learning:** Using `Math.max(...array.map(item => item.value))` on potentially large datasets (like `list.length || 60_000` deals) is a significant performance anti-pattern. First, `.map()` creates a new array in memory, causing unnecessary O(N) allocation and garbage collection overhead. Second, spreading a massive array into function arguments can exceed the JavaScript engine's maximum call stack size, leading to runtime crashes.
**Action:** Replace `Math.max(...array.map(...))` and `.reduce()` chains with a single-pass `for...of` loop when calculating statistics (like max values or sums) over large lists. This ensures O(1) memory complexity and robust execution regardless of array size.
