...
## 2026-09-26 - [Avoid Re-evaluating Minimums on Sorted Arrays]
**Learning:** While replacing `[...arr].sort(...)[0]` with an O(N) minimum-finding function like `getCheapestDeal` is a great optimization for unsorted data, it is a regression to apply it to an array that has *already* been sorted. If an array is already sorted ascending by price, accessing `arr[0]` is an O(1) operation, whereas calling `getCheapestDeal` on it takes O(N) time and performs unnecessary comparisons.
**Action:** Before replacing array accesses with minimum-finding utility functions, verify whether the array is already sorted. Only apply minimum-finding algorithms to unsorted collections to eliminate unnecessary O(N log N) sorting overhead without introducing redundant O(N) traversals.
