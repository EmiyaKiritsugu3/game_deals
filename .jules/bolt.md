...
## 2024-10-24 - [Next.js Fetch Caching in Server Components]
**Learning:** Third-party API calls (like fetching store configurations) on dynamic routes (like search) can block rendering and trigger rate limits if not cached. Next.js does not cache dynamic route fetches by default.
**Action:** Use Next.js extended fetch with `{ next: { revalidate: <seconds> } }` for rarely changing data in Server Components, eliminating redundant network overhead and preventing rate-limiting on high-traffic endpoints.
