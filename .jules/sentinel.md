...
## 2026-08-21 - URL Allowlist Bypass via Protocol Injection
**Vulnerability:** The `isDomainAllowed` check in `src/app/out/[storeId]/[gameSlug]/route.ts` validated only the hostname of user-provided URLs (`ALLOWED_DOMAINS.has(url.hostname)`), leaving the protocol unvalidated.
**Learning:** Malicious protocols (e.g., `javascript:`) can bypass simple hostname allowlists because the URL parser retains the dangerous protocol while correctly interpreting the hostname. This can lead to XSS vulnerabilities if the unvalidated URL is used in a redirect or injected into the DOM.
**Prevention:** When parsing user-provided URLs using `new URL()`, explicitly validate the protocol (e.g., `url.protocol === 'http:' || url.protocol === 'https:'`) in addition to any domain or hostname allowlisting.
